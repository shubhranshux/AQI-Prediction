    ...
    try {
      ...
      if (fixed.current) {
        [j1, j2].forEach(ref => {
          ...
          const transPos = new THREE.Vector3().copy(ref.current.translation());
          const clampedDistance = Math.max(0.1, Math.min(1, ref.current.lerped.distanceTo(transPos)));
          ref.current.lerped.lerp(transPos, delta * (minSpeed + clampedDistance * (maxSpeed - minSpeed)));
        });
      ...
      console.error("Lanyard useFrame Error:", e);
    }
  });
